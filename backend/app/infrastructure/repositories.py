...
class UserRepository:
    ...
    async def save(self, user: User) -> User:
        ...
        await self.session.execute(query, {
            "id": str(user.id), "email": user.email, "name": user.name, "created_at": user.created_at
        })
        return user
...
class OrderRepository:
    ...
    async def save(self, order: Order) -> Order:
        # Исправленный запрос: убран лишний подзапрос в ON CONFLICT
        query_order = text("""
        ...
        # Удаляем старые позиции и вставляем новые (синхронизация)
        await self.session.execute(
            text("DELETE FROM order_items WHERE order_id = :id"), {"id": str(order.id)}
        )
    ...
    async def find_by_id(self, order_id: uuid.UUID) -> Optional[Order]:
        ...
        items_res = await self.session.execute(
            text("SELECT id, product_name, price, quantity FROM order_items WHERE order_id = :id"), {"id": str(order_id)}
        )
        for r in items_res.mappings().all():
            item = OrderItem(
                id=r['id'], product_name=r['product_name'], price=r['price'], quantity=r['quantity'], order_id=order_id
            )
        ...
        for r in hist_res.mappings().all():
            change = OrderStatusChange(id=r['id'], status=OrderStatus(r['name']), changed_at=r['changed_at'])
